...
unsigned long ubloxBaudRates[8]  = { 4800, 9600, 19200, 32400, 57600, 115200, 230400, 460800 };
...
void gps_setup() {
    ...
    int i;
    for(i = 0; i< sizeof(ubloxBaudRates); i++){
        DEBUG_MSG("trying gps at baudrate: %d\n", ubloxBaudRates[i] );
        _serial_gps.begin(ubloxBaudRates[i], SERIAL_8N1, GPS_RX_PIN, GPS_TX_PIN);
        if(myGPS.begin(_serial_gps)){
            DEBUG_MSG("GPS ready at baud rate: %d\n", ubloxBaudRates[i] );
            break;
        }
    }
